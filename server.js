...
// SAFE Binance API with Rate Limit Protection
class BinanceAPI {
    ...
    static async makeRequest(endpoint, method, apiKey, secret, params = {}, useTestnet = false) {
        try {
            ...
            if (rateLimit.bannedUntil > Date.now()) {
                ...
                throw new Error(`⚠️ IP BANNED for ${minutesLeft} more minutes. Please use Testnet mode or wait.`);
            }
            ...
            const baseUrl = useTestnet ? this.testnetUrl : this.baseUrl;
            ...
            if (usedWeight) {
                ...
                if (weight > 1000) {
                    ...
                    console.log(`⚠️ WARNING: Near rate limit! (Warning #${rateLimit.warningCount})`);
                    if (rateLimit.warningCount >= 3) {
                        console.log('🛑 Multiple warnings! Forcing 60 second pause...');
...
app.post('/api/connect', async (req, res) => {
    ...
    try {
        const verification = await BinanceAPI.verifyApiKey(apiKey, secretKey, useTestnet);
        ...
        const balance = await BinanceAPI.getAccountBalance(apiKey, secretKey, useTestnet);
...
app.post('/api/startTrading', async (req, res) => {
    ...
    // For real accounts, verify balance
...
app.post('/api/tradingUpdate', async (req, res) => {
    ...
    // SAFE TRADING: 60 seconds minimum between trades
...
app.listen(PORT, '0.0.0.0', () => {
    console.log('\n' + '='.repeat(60));
    console.log('🌙 HALAL AI TRADING BOT - SAFE RATE LIMITED VERSION');
    console.log('='.repeat(60));
    console.log(`✅ Server running on port: ${PORT}`);
    console.log(`✅ SAFE MODE: 60 seconds between trades`);
    console.log(`✅ 2 second delay between API calls`);
    console.log(`✅ Rate limit protection enabled`);
    console.log(`✅ Auto backoff on 429 errors`);
    console.log(`✅ IP ban detection active`);
    console.log('='.repeat(60) + '\n');
});
